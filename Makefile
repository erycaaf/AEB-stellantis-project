...
#
# Targets (ASIL-D V&V — CAN module):
#   make mcdc-can          — MC/DC coverage (gcc-14 + gcovr, merged nominal + structural + fault)
...
# Per-target V&V report directory — keeps each module stack from
# overwriting the others when invoked in the same workflow.
...
fault-decision:
	...
	$(CC) $(CFLAGS) -O0 -g -o $(VV_REPORT_DIR)/fault_injection/test_decision_fault \
		$(SRC_DECISION_FAULT_TEST) $(LDFLDS)
...
# ── ASIL-D V&V targets (CAN interface) ──────────────────────────────────
#
# mcdc-can compiles three coverage binaries (nominal + structural + fault)
# and runs them sequentially with ; so all three always execute regardless
# of individual exit codes. gcovr then merges the .gcda from all three
# binaries into a single Cobertura XML — this is the merged figure that
# Gate 1 in vv-can.yml parses for ISO 26262-6 Table 12 MC/DC conformance.
...
mcdc-can:
	...
	@grep -E "Results|passed" $(VV_REPORT_DIR)/coverage_mcdc/run_struct.log  || true
	@echo "--- Fault suite ---"
	@grep -E "Results|passed" $(VV_REPORT_DIR)/coverage_mcdc/run_fault.log   || true
	$(GCOVR) --root . \
		--gcov-executable $(GCOV) \
		--filter 'src/communication/aeb_can\.c' \
		--search-path $(VV_REPORT_DIR)/coverage_mcdc \
		--html-details $(VV_REPORT_DIR)/coverage_mcdc/report.html \
		--cobertura   $(VV_REPORT_DIR)/coverage_mcdc/coverage.xml \
		--txt         $(VV_REPORT_DIR)/coverage_mcdc/coverage_summary.txt \
		--print-summary
	@$(GCOV) -b -c \
		$(VV_REPORT_DIR)/coverage_mcdc/test_can_cov-aeb_can.gcno \
		> $(VV_REPORT_DIR)/coverage_mcdc/gcov_summary.txt 2>&1 || true
	@mv -f aeb_can.c.gcov $(VV_REPORT_DIR)/coverage_mcdc/ 2>/dev/null || true
	@echo ""
	@echo "Artefacts in $(VV_REPORT_DIR)/coverage_mcdc/"

fault-can:
	@mkdir -p $(VV_REPORT_DIR)/fault_injection
	$(CC) $(CFLAGS) -O0 -g -o $(VV_REPORT_DIR)/fault_injection/test_can_fault \
		$(SRC_CAN_FAULT_TEST) $(LDFLAGS)
	@$(VV_REPORT_DIR)/fault_injection/test_can_fault \
...
html-can:
	...
	# Coverage HTML — produced by gcovr --html-details in mcdc-can.
	# Fail loudly if the report bundle is missing: an empty publish
	# would go green with no signal.
	@if [ ! -s $(VV_REPORT_DIR)/coverage_mcdc/report.html ]; then \
		echo "html-can: missing coverage_mcdc/report.html — run mcdc-can first"; \
		exit 1; \
	fi
...
vv-can: mcdc-can fault-can memory-can misra-can html-can
	@echo ""
	@echo "=== CAN V&V stack complete — artefacts in $(VV_REPORT_DIR)/ ==="
